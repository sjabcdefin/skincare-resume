...
class SkincareResumesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create skincare_resume' do
    assert_difference('SkincareResume.count') do
      post skincare_resumes_url, params: {
        skincare_resume: {
          status: @skincare_resume.status,
          user_id: @skincare_resume.user_id,
          uuid: @skincare_resume.uuid
        }
      }
    end
  ...
  test 'should update skincare_resume' do
    patch skincare_resume_url(@skincare_resume), params: {
      skincare_resume: {
        status: @skincare_resume.status,
        user_id: @skincare_resume.user_id,
        uuid: @skincare_resume.uuid
